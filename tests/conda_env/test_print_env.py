...
class EnvironmentAndAliasesTestCase(unittest.TestCase):
    ...
    def test_environment_and_aliases_fish(self):
        ...
        try:
            ...
            activate = print_env('activate', self.ENVIRONMENT, {})
            assert activate == textwrap.dedent(
                '''
                set -gx ANY_LIST_REALLY "something1:something2:$ANY_LIST_REALLY"
                set -gx PATH mypath1 mypath2 mypath3 $PATH
                set -gx SINGLE_INT "200"
                set -gx SINGLE_VAR "single_value"
                '''
            ).lstrip()
            ...
            deactivate = print_env('deactivate', self.ENVIRONMENT, {})
            assert deactivate == textwrap.dedent(
                '''
                set -e ANY_LIST_REALLY
