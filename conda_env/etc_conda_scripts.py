...
def write_activate_deactivate(env, prefix):
    ...
    if sys.platform == 'win32':
        __create_activate_and_deactivate_scripts(
            activate_dir,
            conda_dir,
            deactivate_dir,
            env,
        ...
        __create_activate_and_deactivate_scripts(
            ...
            ext='.fish',
            rm='rm',
            set_temp_file='set TEMP_FILE (%s)',
            source='source',
            temp_file='$TEMP_FILE',
        )


def __create_activate_and_deactivate_scripts(activate_dir, conda_dir, deactivate_dir, env, ext, rm,
                                             set_temp_file, source, temp_file):
